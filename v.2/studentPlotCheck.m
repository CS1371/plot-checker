...
funExists = exist(funName); %#ok<EXIST> %Check for the existence of the function in your directory
if funExists
    feval(funName, varargin{:}); %Execute the function and all inputs of that function
else
    error('Could not locate function %s', funName);
end
...
for i = 1:length(student) % i represent the current subplot of comparison
    for j = 1:length(fieldnames(student)) % go through all properties in the structure
        switch fields{j}
            ...
            case {'XLabels','YLabels','ZLabels','Title','Marker','LineStyle'}
...
elseif isnumeric(studIN) && isnumeric(solnIN)
    ...
    if r > 1 && r2 > 1
        ...
        out3 = all(ismembertol(studINdiff,solnINdiff,'ByRows',true));
    else
        out3 = true;
    end
